...
class Logo {
  ...
  validateHex(hexColor, shorthand) {
    // If shorthand true, then it'll equal first cond, e.g. "#B5F"
    const hexColorPattern = shorthand
      ? /^#[0-9A-Fa-f]{3}$/
      : /^#[0-9A-Fa-f]{6}$/;
  ...
  validateColor(color) {
    ...
    if (givenColor.startsWith("#")) {
      ...
      if (givenColor.length === 4) {
        ...
        throw new Error(
          'Invalid hex color code. It should start with "#" and be 6 characters long (3 for shorthand).'
        );
      }
      // Keyword validation
    } else if (!colors.hasOwnProperty(givenColor)) {
      throw new Error(
        "Invalid color keyword. Please use one of the 140 defined CSS color keywords"
      );
    }
  }
